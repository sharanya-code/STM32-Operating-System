/**
  ******************************************************************************
  * @file           : Kernel.c
  * @brief          : Kernel related function definitinos
  ******************************************************************************
  * @attention
  *
  *
...
static task_t scheduler(void)
{
	 ...
	 */
	/*
    task_t next_task = (task_t) ((kernel_config.running_task + 1) % MAX_TASKS);

    while (kernel_config.TCBS[next_task].state != READY) {
        next_task = (next_task + 1) % MAX_TASKS;
        if(next_task == kernel_config.running_task){
...
int osCreateDeadlineTask(int deadline, TCB* task){
	...
	*create_tcb= *task;
	...
	transfer_function(create_tcb->p_stack_mem, create_tcb->tid);
